...
class Serializer(base.Serializer):
    ...
    def handle_field(self, obj, field):
        ...
        self._fields[field.name] = smart_unicode(getattr(obj, field.name),
            strings_only=True)
        fname = field.name
        if fname in self.display:
            try:
                dname = eval('''obj.get_%s_display''' % fname)()
                self._fields[fname] = smart_unicode(dname)
            except AttributeError:
                self._fields[field.name] = smart_unicode(getattr(obj, field.name), strings_only=True)
        else:
            self._fields[field.name] = smart_unicode(getattr(obj, field.name), strings_only=True)
