...
class PageManager(models.Manager):
    ...
    def published(self, user=None, now=datetime.datetime.now()):
        queryset = self.all()
...
class PageContent(models.Model):
    ...
    def save(self, force_insert=False, force_update=False):
        ...
        if self.page.toc:
            try:
                pat = re.compile('<a\s+name="(.*)">', re.I)
                cnt = [ "<li><a href=\"#%s\">%s</a></li>" % (m.group(1), m.group(1)) for m in pat.finditer(self.content) ]
                self.toc = "<ul>\n\t%s\n</ul>" % ("\n\t".join(cnt))
            except TypeError:
                pass        
        super(PageContent, self).save(force_insert, force_update)
