...
@register.filter(name='cms_toc')
def cms_tableofcontents(content, autoescape=None):

    import re
    pat = re.compile('<a\s+name="(.*)"></a>', re.I)
    cnt = []

    for m in pat.finditer(content['']):
        cnt.append("\t<li><a href=\"#%s\">%s</a></li>\n" % (m.group(1), m.group(1)))

    str = "<ul>%s</ul>" % ( ''.join(cnt) )

    return mark_safe(str)
cms_tableofcontents.needs_autoescape = True
